...
const components = {
  h1: (props: HTMLAttributes<HTMLHeadingElement>) => (
    <h1 className="mt-8 mb-4 font-serif text-3xl font-bold text-orange-100 tracking-tight" {...props} />
  ),
  h2: (props: HTMLAttributes<HTMLHeadingElement>) => (
    <h2 className="mt-10 mb-4 font-serif text-2xl font-bold text-orange-50 tracking-tight" {...props} />
  ),
  h3: (props: HTMLAttributes<HTMLHeadingElement>) => (
    <h3 className="mt-8 mb-4 font-serif text-xl font-bold text-orange-50 tracking-tight" {...props} />
  ),
  ...
  pre: (props: HTMLAttributes<HTMLPreElement>) => (
    <pre className="bg-black/50 p-6 rounded-xl border border-white/10 font-mono text-sm text-gray-300 my-8 overflow-x-auto" {...props} />
  ),
  code: (props: HTMLAttributes<HTMLElement>) => (
    <code className="bg-black/30 rounded px-1.5 py-0.5 font-mono text-sm text-orange-200" {...props} />
  ),
  a: (props: any) => {
    const href = props.href;
    if (href?.startsWith('/')) {
      return (
        <Link href={href} {...props} className="text-orange-400 hover:text-orange-300 underline underline-offset-4">
          {props.children}
        </Link>
      );
    }
